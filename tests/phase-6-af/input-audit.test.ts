...
/**
 * Phase 6-AF â€” Input Composition and Relationship Structure Audit
 *
...
describe("PHASE 6-AO-V9 reader compatibility — input-audit extraction", () => {
  ...
  it("extracts the complete multi-segment SYS_V5 prompt and matches the frozen V5 hash", () => {
    const { system, userTemplate } = readIdentityPromptSource();
    ...
    // Legacy reader semantics intact: user template shape preserved.
    expect(userTemplate.startsWith("NEW OBSERVATION")).toBe(true);
    expect(userTemplate.trimEnd().endsWith("JSON only:")).toBe(true);
  ...
  it("reports the frozen verifier model constant unchanged", () => {
    expect(readIdentityPromptSource().modelConstant).toBe("qwen2.5:3b");
  });
});
...
/**
 * PHASE 6-AO-V14 — EMBEDDING MODEL EVALUATION WITH QUERY PREFIX
 * =============================================================================
 * PURPOSE (measurement only): evaluate whether applying the model-recommended
 * query prefix to mxbai-embed-large:latest inputs improves SAME-pair retrieval
 * at the FIXED production identity boundary 0.85 while preserving the SYS_V5
 * verifier safety contract.
 *
 * EXPERIMENTAL DESIGN:
 *   - Baseline embed = nomic-embed-text:latest (production), dim 768.
 *   - Candidate embed = mxbai-embed-large:latest, dim 1024.
 *   - Fidelity check: bare-text candidate must reproduce V13 TP=17.
 *   - Prefix experiment: candidate with query prefix (default "query: ").
 *   - Verifier = SYS_V5 (hash-pinned), threshold 0.85, candidate count 8,
 *     options {temp 0, num_predict 256, top_p 0.9, timeout 30000}.
 *
 * INDEPENDENT VARIABLE:
 *   - V14_CANDIDATE_QUERY_PREFIX: input format (bare text vs prefixed query)
 *
 * FROZEN CONTROLS:
 *   - Threshold = 0.85
 *   - Dataset = tests/phase-6-ao/dataset.json (SHA-pinned)
 *   - Verifier = SYS_V5 (hash-pinned)
 *   - Baseline = nomic-embed-text:latest
 *
 * GATES:
 *   - PREFIX_FIDELITY: bare-text TP must equal 17
 *   - RECALL_GAIN >= +15pp
 *   - FCR <= 5%
 *   - REPEATABILITY >= 18/20
 *   - PREFIX_BENEFIT: TP_prefix >= TP_bare
 *
...
const OLLAMA_URL =
  process.env.V14_OLLAMA_URL || process.env.OLLAMA_BASE_URL || "http://127.0.0.1:11434";
const API_TAGS = `${OLLAMA_URL}/api/tags`;
const API_EMBED = `${OLLAMA_URL}/api/embed`;
const API_CHAT = `${OLLAMA_URL}/api/chat`;
const OLLAMA_AUTH = process.env.V14_OLLAMA_AUTH ?? "";
...
const CANDIDATE_MODEL = process.env.V14_CANDIDATE_MODEL ?? "";
...
const RESULTS_PATH = path.join(RESULTS_DIR, "v14-embedding-prefix-evaluation.json");
...
async function embed(text: string, model: string): Promise<number[]> {
  ...
  const inputText =
    model === state.candidateModel && state.candidateQueryPrefix
      ? state.candidateQueryPrefix + text
      : text;
  const ctl = new AbortController();
  const t = setTimeout(() => ctl.abort(), 10000);
  let res: Response;
...
const state: {
  ...
  gates: null,
  fidelityPass: null,
  candidateQueryPrefix: process.env.V14_CANDIDATE_QUERY_PREFIX ?? DEFAULT_QUERY_PREFIX,
};

function block(reason: string) {
  state.reason = reason;
  state.status = "BLOCKED";
  console.log("V14 STATUS=BLOCKED REASON=" + reason);
}

async function runExperiment(): Promise<(RunResult & { recallFixed: number }) | null> {
  if (state.status === "BLOCKED" || !state.contract) {
    console.log("V14 MAIN-EXPERIMENT SKIPPED (blocked): " + (state.reason ?? "unknown"));
    return null;
  }
  ...
  return { ...metrics, recallFixed: metrics.tp / FROZEN_SAME_DENOMINATOR };
}

describe("PHASE 6-AO-V14 — embedding model evaluation with query prefix (zero-write, production-frozen)", () => {
  ...
  it("preflight: live /api/tags + embedding probe; resolve candidate (BLOCK if absent)", { timeout: 30_000 }, async () => {
    ...
    console.log(
      "V14 candidate=" + installedTag + " embeddingCapable=" + state.embeddingCapable[installedTag]
    );
  });

  it("extracts the production contract; pins SYS_V5 hash and production threshold 0.85", () => {
    if (state.status === "BLOCKED") {
      console.log("V14 contract check SKIPPED (blocked): " + (state.reason ?? "unknown"));
      return;
    }
    ...
    console.log("V14 contract pinned model=" + extracted.model + " promptSha256=" + extracted.promptHash);
  });

  it("fidelity check: bare-text mxbai reproduces V13 TP=17", { timeout: 1_800_000 }, async () => {
    if (state.status === "BLOCKED" || !state.contract) {
      console.log("V14 FIDELITY CHECK SKIPPED (blocked): " + (state.reason ?? "unknown"));
      state.fidelityPass = false;
      return;
    }
    const savedPrefix = state.candidateQueryPrefix;
    state.candidateQueryPrefix = "";
    ...
    state.pairs = [];
    state.band = [];
    state.uncertainTotal = 0;
    state.transportRetries = 0;
    state.repeatability = null;
    state.gates = null;
    const result = await runExperiment();
    state.candidateQueryPrefix = savedPrefix;
    if (!result) {
      console.log("V14 FIDELITY CHECK FAILED: " + (state.reason ?? "unknown"));
      state.fidelityPass = false;
      return;
    }
    const tpFidelity = result.tp;
    if (tpFidelity !== 17) {
      block("FIDELITY_FAIL: expected TP=17, got TP=" + tpFidelity);
      state.fidelityPass = false;
      return;
    }
    state.fidelityPass = true;
    console.log("V14 FIDELITY PASS TP=" + tpFidelity);
  });

  it(
    "prefix experiment: mxbai with query prefix",
    { timeout: 1_800_000 },
    async () => {
      if (!state.fidelityPass) {
        console.log("V14 PREFIX EXPERIMENT SKIPPED (fidelity check failed)");
        return;
      }
      if (state.status === "BLOCKED" || !state.contract) {
        console.log("V14 PREFIX EXPERIMENT SKIPPED (blocked): " + (state.reason ?? "unknown"));
        return;
      }
      state.candidateQueryPrefix = process.env.V14_CANDIDATE_QUERY_PREFIX ?? DEFAULT_QUERY_PREFIX;
      state.status = "PENDING";
      state.reason = undefined;
      state.anchorOk = null;
      state.run = null;
      ...
      const result = await runExperiment();
      if (!result) {
        console.log("V14 PREFIX EXPERIMENT BLOCKED: " + (state.reason ?? "unknown"));
        return;
      }
      const tpCandidate = result.tp;
      const recallCandidateFixed = result.recallFixed;
      const recallGainPP = (recallCandidateFixed - BASELINE_FIXED_RECALL) * 100;
      const fcr = result.falseCorroborationRate;
      const gateRecall = recallGainPP >= 15 ? "PASS" : "FAIL";
      const gateSafety =
        fcr === null ? "PASS (no SAME verdicts — no false corroboration possible)"
        : fcr <= 0.05 ? "PASS" : "FAIL";
      const gateRepeat =
        state.repeatability === null || (state.repeatability as { allPass: boolean | null }).allPass === null
          ? "PASS (vacuous: no newly-promoted band cells)"
          : (state.repeatability as { allPass: boolean }).allPass ? "PASS" : "FAIL";
      const overall =
      ...
      state.gates = {
        ...
        fcr,
        pair005: {
          baseline: ANCHOR_PAIR_005_EXPECTED,
          candidate: Number(
            (state.pairs.find((p) => p.pairId === "pair-005")?.candidateSimilarity as number ?? 0).toFixed(6)
          ),
          candidateAboveThreshold:
            (state.pairs.find((p) => p.pairId === "pair-005")?.candidateSimilarity as number ?? 0) >=
            PRODUCTION_THRESHOLD,
        },
        pair041: {
          baseline: ANCHOR_PAIR_041_EXPECTED,
          candidate: Number(
            (state.pairs.find((p) => p.pairId === "pair-041")?.candidateSimilarity as number ?? 0).toFixed(6)
          ),
          candidateAboveThreshold:
            (state.pairs.find((p) => p.pairId === "pair-041")?.candidateSimilarity as number ?? 0) >=
            PRODUCTION_THRESHOLD,
        },
      };
      console.log("V14 GATES " + JSON.stringify(state.gates));
    }
  );

  it("persists structured results once, to the previously-absent V14 slot", () => {
    fs.mkdirSync(RESULTS_DIR, { recursive: true });
    const payload = {
      phase: "PHASE 6-AO-V14",
      title: "Embedding model evaluation with query prefix (candidate vs nomic-embed-text:latest)",
      status: state.status,
      ...
      prefix: {
        candidateQueryPrefix: state.candidateQueryPrefix,
        fidelityPass: state.fidelityPass,
      },
      anchor: {
      ...
      pairs: state.pairs,
      predecessor: {
        v13: "results/v13-embedding-model-evaluation.json (COMPLETE, FAIL — bare-text mxbai)",

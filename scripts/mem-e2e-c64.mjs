...
console.log('EMAIL:', Buffer.from(emailB64, 'base64').toString('utf8'));
console.log('SECRET_LEN:', Buffer.from(secretB64, 'base64').toString('utf8').length);

console.log('\n=== SIGN IN ===');
const sb = createClient(url, anon);
const { data: signInData, error: signInErr } = await sb.auth.signInWithPassword({
  email: Buffer.from(emailB64, 'base64').toString('utf8'),
  password: <REDACTED> 'base64').toString('utf8'),
});
